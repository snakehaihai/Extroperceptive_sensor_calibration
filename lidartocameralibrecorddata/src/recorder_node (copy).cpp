...
int main(int argc, char **argv)
{
...
#ifdef ETH_LIDAR
    // --------------------------------------------------------
    //crete ACM LDIAR objects
    CHokuyoURG laser_acm;
    laser_acm.setSerialPort("ttyACM1");
    laser_acm.setIntensityMode(false);
    laser_acm.setReducedFOV(LIDARfov);
    if (laser_acm.turnOn())
...
#ifdef ETH_LIDAR
    //crete ETH LDIAR objects
    CHokuyoURG laser_ETH;
    laser_ETH.setIPandPort("192.168.0.10", 10940);
    laser_ETH.setIntensityMode(false);
    laser_ETH.setReducedFOV(LIDARfov);
    if (laser_ETH.turnOn())
        printf("[TEST] Initialization OK!\n");
    else {
        printf("[TEST] Initialization failed!\n");
    ...
    //creating saving method
        ...
        while (!win3D.keyHit()||!mrpt::system::os::kbhit())	//Push any key to exit // win3D.isOpen()
        {
...
#ifdef ACM_LIDAR
            bool thereIsObservation_LIDARACM, hardError_LIDARACM;
            CObservation2DRangeScan obs_LIDARACM;
            laser_acm.doProcessSimple(thereIsObservation_LIDARACM, obs_LIDARACM, hardError_LIDARACM);
            if (hardError_LIDARACM) printf("[Debug Hardware issue] Hardware error=true!!\n");
            if (thereIsObservation_LIDARACM) {
                obs_LIDARACM.sensorLabel="HOKUYO1";
                obs_LIDARACM.sensorPose = CPose3D(0, 0, 0);
                f_out << obs_LIDARACM;
            ...
            //sensor_msgs::LaserScan laserscan2;
#ifdef ETH_LIDAR
            bool thereIsObservation_LIDARETH, hardError_LIDARETH;
            CObservation2DRangeScan obs_LIDARETH;
            laser_ETH.doProcessSimple(thereIsObservation_LIDARETH, obs_LIDARETH, hardError_LIDARETH);
            if (hardError_LIDARETH) printf("[Debug Hardware issue] Hardware error=true!!\n");
            if (thereIsObservation_LIDARETH) {
                obs_LIDARETH.sensorLabel="HOKUYO2";
                obs_LIDARETH.sensorPose = CPose3D(0, 0, 0);
                f_out << obs_LIDARETH;
            ...
            //obs_LIDARETH.timestamp=mrpt::system::time_tToTimestamp(laserscan1.header.stamp.sec + laserscan1.header.stamp.nsec * 1e-9);
