...
void calib_LRFs_rawlog_ini(const string &INI_FILENAME, const string &override_rawlog_file) {
    ...
    int M_num_LRFs = iniFile.read_int("calib-LRFs", "M_num_LRFs", 2,  /*Force existence:*/ true);
...
std::cout<<"runhere2"<<std::endl;
            ...
            {
                std::cout << "MRPT exception caught Error at ransac line: " << e.what() << std::endl;
            ...
            std::cout<<"Successfully ransaced"<<std::endl;
            // Generate vCOs.
...
//#endif
    ...
    //----------------------------------------------------------------------------------------------------------------------------------------
    std::cout<<"Successfully Find all corner observation"<<std::endl;
//    vCOs.resize(400);
...
void calib_LRFs_rawlog_ini(const string &INI_FILENAME, const string &override_rawlog_file) {
    ...
    const double threshold_line=0.03;
    const size_t min_inliers_line=100;
    set<unsigned> idx_estim_LRFs;
    //#include "geometry_msgs/Pose.h"

    //sensor_msgs::Pose;
    std::vector<string> LRF_labels(M_num_LRFs);
    map<unsigned,geometry_msgs::Pose> initial_Poses;
    initial_Poses[0].position.x=0;
    initial_Poses[0].position.y=0;
    initial_Poses[0].position.z=0;
    initial_Poses[0].orientation.x=0;
    initial_Poses[0].orientation.y=0;
    initial_Poses[0].orientation.z=0;

    initial_Poses[1].position.x=0;
    initial_Poses[1].position.y=0;
    initial_Poses[1].position.z=0;
    initial_Poses[1].orientation.x=0;
    initial_Poses[1].orientation.y=0;
    initial_Poses[1].orientation.z=0;
